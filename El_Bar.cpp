...
void procesarDatos(vector<string>& resultados);
...
int main() {
    ...
    vector<string> resultados; // para mostrar resultados en pantalla
    bool redraw = true;
    bool running = true;
    ...
    while (running) {
        ...
        if (ev.type == ALLEGRO_EVENT_DISPLAY_CLOSE) {
            ...
            if (puntoEnRect(mx, my, btnProcesar.x, btnProcesar.y, btnProcesar.w, btnProcesar.h)) {
                resultados.clear();
                procesarDatos(resultados); // procesar datos del archivo
                redraw = true;
            }
        ...
        if (redraw && al_is_event_queue_empty(queue)) {
            ...
            // mostrar resultados
            int y_texto = 80;
            al_draw_multiline_text(font, al_map_rgb(255, 255, 255), 30, y_texto, 580, 22, ALLEGRO_ALIGN_LEFT,
                resultados.empty() ? "Presiona 'Procesar datos' para comenzar." : "");

            for (const auto& linea : resultados) {
                al_draw_text(font, al_map_rgb(255, 255, 255), 30, y_texto, ALLEGRO_ALIGN_LEFT, linea.c_str());
                y_texto += 24;
            }
...
void procesarDatos(vector<string>& resultados) {
    ...
    if (fopen_s(&bar, "bar.txt", "r") != 0) {
        resultados.push_back("Error: No se pudo abrir 'bar.txt'");
        return;
    }
    FILE* salida;
    if (fopen_s(&salida, "salida.txt", "w") != 0) {
        resultados.push_back("Error: No se pudo crear 'salida.txt'");
        fclose(bar);
        return;
    }
    ...
    if (resultados.empty()) {
        resultados.push_back("No se encontraron datos para procesar.");
    }
}
